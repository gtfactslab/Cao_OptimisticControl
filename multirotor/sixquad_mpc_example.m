...
global  goal_rect unsafe_rects Ts blocking_interval
...
fprintf(results_file, 'Run\tSeed\tOpt\tPes\tBest\n');
...
for r = 1:num_runs
    ...
    % optimal trajectory given full knowledge
    nlobjBest = nlmpc(6,1,2);
    nlobjBest.Ts = Ts/blocking_interval; % these should set prediction horizon to match Ts of control actions
    nlobjBest.PredictionHorizon = control_actions*blocking_interval;
    nlobjBest.ControlHorizon = [ones(1,control_actions) * blocking_interval];
    % model system dynamics
    nlobjBest.Model.StateFcn = @(x, u) actual_system(0, x, u);
    % If true, MATLAB automatically discretizes the model for optimization
    % using Ts, else Ts is the sample time
    nlobjBest.Model.IsContinuousTime = true; 
    nlobjBest.Model.OutputFcn = @(x,u) x(1);
    nlobjBest.Optimization.SolverOptions.Display = 'iter';
    nlobjBest.Optimization.SolverOptions.Algorithm = 'sqp'; 
    % interior-point (default), trust-region-reflective, sqp, sqp-legacy, active-set
    nlobjBest.Optimization.SolverOptions.UseParallel = true;
    nlobjBest.Optimization.SolverOptions.MaxIterations = 600; % default 400
    nlobjBest.MV(1).Min = -u_max(1);
    nlobjBest.MV(1).Max = u_max(1);
    nlobjBest.MV(2).Min = -u_max(2);
    nlobjBest.MV(2).Max = u_max(2);
    validateFcns(nlobjBest, x0, u0B);
    ...
    for i = 1:100
        ...
        if i > 1
            ...
            nlobjBest.Optimization.CustomIneqConFcn = @leq_constraints_hitgoalB;
        else
            nlobj.Optimization.CustomIneqConFcn = @leq_constraints_goal;
            nlobjBest.Optimization.CustomIneqConFcn = @leq_constraints_goalB;
        end
        nlobj.Optimization.CustomEqConFcn = @eq_constraints;
        ...
        if ~in_goal(2)
            ...
            if ~exitFlag
                fprintf('Reaching Goal Infeasible (Pes).\n');
    ...
    %             u0 = u02;
    %         end
            % question: should this be 0 or -3? if mean is not included then
            % actual observations are also ignored
            nlobj.MV(3).Min = 0;
            nlobj.MV(4).Min = 0;
            nlobj.Model.StateFcn = @(x, u) sixquad_emb_mpc_vardist(x, u, obs_y, obs_z);
        ...
        if ~in_goal(3)
            ...
            [mvB, optB, infoB] = nlmpcmove(nlobjBest, cur_xB, u0B);
            exitFlag = infoB.ExitFlag >= 0;
            % if reaching goal infeasible, check if any actions are stored and perform the next one
            if ~exitFlag
                fprintf('Reaching Goal Infeasible (Best).\n');
    ...
    %     hold off
        ...
        if all(in_vals + 1e-4 >= 0)
            fprintf('(Best) Goal Reached.\n');
            in_goal(3) = true;
        else
...
function leq = leq_constraints_goalB(X, U, e, data)
    leq = [];
    leq = [leq; end_in_goal([X, X], U, e, data, 0)];
end

function leq = leq_constraints_hitgoalB(X, U, e, data)
    leq = [];
    leq = [leq; hit_goal([X, X], U, e, data, 0)];
end
...
% tic
    ...
    leq = 0.4 - prob; % want probability at least 0.x
% toc
